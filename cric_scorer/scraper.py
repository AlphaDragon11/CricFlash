    ...
    for detail in details:
        ...
        if summary is not None:
            ...
            if match_anchor:
                match_url = match_anchor['href']
                live_team_details['scoreboard'] = get_individual_match_scoreboard(match_url)
                live_team_details['additional_details'] = get_additional_match_details(match_url)  # Add this line to fetch additional match details
            live_match[teams[0] + " vs " + teams[1]] = live_team_details
    return live_match

def get_additional_match_details(url):
    match_url = "https://www.cricbuzz.com" + url
    page = requests.get(match_url)
    soup = BeautifulSoup(page.content, "html.parser")
    additional_details = soup.find_all("div", class_="cb-col cb-col-100 cb-ltst-wgt-hdr")
    details = []
